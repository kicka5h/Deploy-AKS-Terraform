...
resource "azurerm_kubernetes_cluster" "aks_dr" {
  ...
  name                = "${var.env}-KubeAKS-dr"
  location            = azurerm_resource_group.kuberg_dr[0].location
  resource_group_name = azurerm_resource_group.kuberg_dr[0].name
  dns_prefix          = "${var.env}-KubeAKS-dr"
  kubernetes_version  = var.aks_cluster_version

  default_node_pool {
    name                 = "default"
    orchestrator_version = var.aks_cluster_version
    node_count           = var.dr_aks_node_count
    vm_size              = var.dr_aks_vm_size
    vnet_subnet_id       = data.azurerm_subnet.vnet_subnet1_dr[0].id
    enable_auto_scaling  = var.aks_auto_scaling_enabled
    min_count            = var.aks_auto_scaling_min_count
    max_count            = var.aks_auto_scaling_max_count
  }
